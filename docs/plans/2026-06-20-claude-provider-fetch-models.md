...
1. `cd src-tauri && cargo check` 通过；`cargo test` 跑过 `derive_model_list_candidates` 单测。
2. 前端类型检查 / 构建通过（项目现有 vitest + tsc 流程）。
3. 手动：启动应用 → 添加 Claude 供应商 → 填入真实中转站 base URL + key → 点"拉取模型" → 确认三个输入框出现下拉建议；选一个 → 确认 JSON 配置区对应 env 同步更新；再用错误 URL/Key 验证错误提示。
