...
- 调 `reorderClaudeProviders(orderedIds)`，成功后 `await loadProviders()` 同步；失败回滚（重新 `loadProviders()`）。
...
1. `cd src-tauri && cargo check` 通过；`cargo test` 跑过新增排序单测。
2. 前端类型检查 / 构建通过（现有 vitest + tsc 流程）。
3. 手动：
