...
describe("useThreads UX integration", () => {
  ...
  it("settles background Codex assistant completion without mutating the highlighted thread", async () => {
    vi.useFakeTimers();
    ...
    try {
      ...
      await act(async () => {
        ...
        handlers?.onTurnStarted?.("ws-1", "thread-b", "turn-b");
        handlers?.onAgentMessageCompleted?.({
          workspaceId: "ws-1",
          threadId: "thread-a",
          itemId: "assistant-a",
      ...
      expect(result.current.threadStatusById["thread-b"]?.isProcessing).toBe(true);
  ...
  it("does not use a previous Codex final answer as activation drift evidence for a new live turn", async () => {
    ...
    try {
      ...
      await act(async () => {
        handlers?.onTurnStarted?.("ws-1", "thread-a", "turn-1");
        handlers?.onAgentMessageCompleted?.({
          workspaceId: "ws-1",
          threadId: "thread-a",
        ...
        handlers?.onTurnCompleted?.("ws-1", "thread-a", "turn-1");
      });
      expect(result.current.threadStatusById["thread-a"]?.isProcessing).toBe(false);

      now += 1_000;
      await act(async () => {
        await result.current.sendUserMessageToThread(workspace, "thread-a", "next prompt");
        handlers?.onTurnStarted?.("ws-1", "thread-a", "turn-2");
      });
      expect(result.current.threadStatusById["thread-a"]?.isProcessing).toBe(true);
