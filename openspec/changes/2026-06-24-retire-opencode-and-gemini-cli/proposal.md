...
- **Vendors gemini proxy 整块** (`src-tauri/src/vendors/commands.rs` 1343 行)
- **前端 thread loaders**:
...
10. `src-tauri/src/vendors/commands.rs` (1343, 全 gemini proxy)
...
1. `src-tauri/src/engine/mod.rs`:
   ...
   - `SendMessageParams.agent` / `variant` 2 个字段 + Default 实现
...
return `Ok(json!({ "delegateTo": "send_user_message" }))`,**不构造
`SendMessageParams`**。所以:
- 删 `SendMessageParams.agent` / `variant` 字段**不影响** Codex 路径
- 删 `engine_send_message` 函数签名的 `agent` / `variant` 参数**不影响** Codex
  路径 (Codex arm 不读这俩参数)
...
- `gemini` engine 的 `gemini_proxy_guard.rs` + `vendors/commands.rs:1343` 是
  单独的 proxy 通道, 与 codex 走 `app-server` / claude 走 `--print` 的路径
  完全异构, 维护成本不对等。
...
- **Vendors (gemini proxy)**:
  - `src-tauri/src/vendors/commands.rs` (1343 行) 中 gemini proxy 命令整块拆出
  - `src-tauri/src/vendors/mod.rs` 中 gemini 子模块引用
- **Daemons**:
...
- **风险 3**: opencode/gemini 删了, 但 `vendors/commands.rs` 里 gemini proxy
  1343 行删除会牵连 client 走 `app_server_cli.rs` 的 proxy 路径。**缓解**:
  验证 Codex / Claude 子进程启动不需要 gemini proxy, 通过 mock e2e (Gate 2)
  守门; 删之前跑 `rg -n "vendors::commands::gemini" src-tauri/src` 确认无
  其他调用点。
