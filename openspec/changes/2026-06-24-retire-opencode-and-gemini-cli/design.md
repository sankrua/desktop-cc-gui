...
`vendors/commands.rs` 1343 行 proxy 通道)。
...
- **Vendors proxy**:
  - `src-tauri/src/vendors/commands.rs` 是 gemini CLI 的 proxy 通道, 1343 行
  - 与 `app_server_cli.rs` (Codex) / `claude.rs` (Claude) 完全异构, 是
    gemini 删起来最重的部分
- **Daemon**:
...
或者更简单: 给 `EngineType` 加一个 `#[serde(other)]` 风格的 wrapper, 未知
变体落到 `Codex` 默认。
...
### Decision 7: `vendors/commands.rs` 1343 行 gemini proxy 整块拆出

`grep -n 'gemini\|Gemini' src-tauri/src/vendors/commands.rs` 全删, 留下空
`commands.rs` 只保留 `pub mod` 声明让 `mod.rs` 不报错 (或整文件 `git rm`
+ `mod.rs` 同步删一行)。

**前提验证** (task 0.4): `rg -n "vendors::commands::" src-tauri/src` 确认
无其它模块调用 gemini 路径, 否则阻塞 task 7。

**回滚**: 整块恢复 = git revert 1 file + `mod.rs` 1 行。
...
3. **Stage 2 — 后端 engine 整模块拆出 (中风险)**: `git rm` 8 个 engine
   文件 + `mod.rs` 8 行; `git rm` vendors/commands.rs gemini proxy;
