...
当前 `setEditorContent(nextContent)` 仍保留在 FileViewBody 局部，但后续目标是减少它对父级 render 的影响：

- dirty 状态只发布 boolean，不发布 full content。
- active file declaration anchor 不在每次输入时从 React `content` 全量重新解析。
- preview snapshot 不在编辑 mode 的每次 publish 后自动刷新，除非 live preview 明确开启。
