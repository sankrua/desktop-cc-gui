...
- Composer draft value 的 source of truth 保持在 Composer-local 边界，避免订阅 global streaming / radar / activity tick。
...
2. **Composer isolation**：收窄 `useComposerEditorState` 与 ChatInputBox value path 的订阅范围，input history 改为 background hydration。
...
- streaming 期间 Composer 输入路径不订阅无关 shell 高频 state，`S-CI-50` 和 `S-CI-100-IME` 不退化。
