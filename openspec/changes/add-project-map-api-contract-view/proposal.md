...
- API view 必须支持层级划分，默认按 `协议 -> 模块/包/namespace -> controller/router/service -> endpoint` 聚合展示；默认永远先 group-first，`endpoint <= 30` 可自动展开 endpoint，`endpoint > 50` 必须只渲染 group 与聚合边，不允许全量平铺。
...
- 新增 API contract discovery pipeline：
  ...
  - 再通过语言 adapter 识别 route annotation、decorator、macro、router registration、handler function 和 request/response model。
...
- 新增 `接口 API` tab：
  ...
  - 复用 zoom、layout、reset、inspector 折叠等图谱控制。
  - 默认展示分层 group graph，而不是 endpoint 平铺图。
- 新增 API Inspector：
  - 展示 path / method / protocol / framework / handler / source file。
  - 展示 path/query/header/body/cookie 入参、request schema、status code、content-type、response schema、error response、description、usage scenario。
...
- API graph 必须支持 zoom、reset、layout 切换和 inspector 折叠，交互能力与关系图谱保持一致。
